"""
This Script (stacking 1)
1. grab spectrograms using a map file
2. load saved models (main and unknown)
3. generate winning-class and its probability

Next Scripts (stacking 2 & 3)
4. generate unknown determination and its probability
5. make logical decision based on step 3 and 4
    ...
    for i, data in enumerate(dataloader):
        ...
        if use_gpu:
            ...
            pred_prob = np.append(pred_prob,
                                  outputs.topk(1)[0].data.view(
                                      1, -1).cpu().numpy())
        else:
            pred = np.append(pred,
                             outputs.topk(1)[1].data.view(1, -1).numpy())
            pred_prob = np.append(pred_prob,
                                  outputs.topk(1)[0].data.view(1, -1).numpy())
    return pred, np.exp(pred_prob)

print(">>> Generating testing results...")
test_1 = pd.DataFrame()
test_1['pred'], test_1['pred_prob'] = testModel(testloader)
test_1['fname'] = map_df.path.apply(lambda x: x.split('/')[-1])
print(test_1.head())
test_1_name = '../log/test_1_' + datetime.datetime.now().\
    strftime("_%Y_%m_%d_%H_%M") + ".csv"
test_1.to_csv(test_1_name, index=False)
