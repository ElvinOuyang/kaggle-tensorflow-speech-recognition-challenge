...
batch_size = 250
num_workers = 10
load_model = '../models/unknown_cnn_2018_01_14_19_09_epoch_19_lr_0.005.pt'
use_gpu = True
...
def testModel(dataloader):
    ...
    for i, data in enumerate(dataloader):
        ...
        if use_gpu:
            ...
            unknown_prob = np.append(unknown_prob,
                                     outputs.narrow(1, 0, 1).data.contiguous().
                                     view(1, -1).cpu().numpy())
        else:
            pred = np.append(pred,
            ...
            unknown_prob = np.append(unknown_prob,
                                     outputs.narrow(1, 0, 1).data.contiguous().
                                     view(1, -1).numpy())
    return pred, np.exp(unknown_prob)

print(">>> Generating unknown checker results...")
test_2 = pd.DataFrame()
test_2['pred'], test_2['unknown_prob'] = testModel(testloader)
test_2['fname'] = map_df.path.apply(lambda x: x.split('/')[-1])
print(test_2.head())
test_2_name = '../log/test_2_' + datetime.datetime.now().\
    strftime("_%Y_%m_%d_%H_%M") + ".csv"
test_2.to_csv(test_2_name, index=False)
